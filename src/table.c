...
RID table_insert(Table *table, ItemPtr src, short size) {
    ...
    RID rid;
    if (addr == -1) {
        Block *block = (Block*)get_page(&table->data_pool, table->data_pool.file.length);
        init_block(block);
        idx = new_item(block, src, size);
        hash_table_insert(&table->fsm_pool, block->tail_ptr - block->head_ptr, table->data_pool.file.length - PAGE_SIZE);
        get_rid_block_addr(rid) = table->data_pool.file.length - PAGE_SIZE;
        release(&table->data_pool, table->data_pool.file.length - PAGE_SIZE);
    }
    else {
